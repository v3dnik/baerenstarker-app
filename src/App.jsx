...
function buildPrintHtml(inv) {
 Print / PDF speichern</button><button class="btn-close" id="btnClose">
  <div style="display:flex;justify-content:space-between;align-items:flex-start;border-bottom:3px solid #1a2744;padding-bottom:16px;margin-bottom:24px;">
    <div><img src="${window.location.origin}/logo_full.png" alt="Logo" style="height:65px;width:auto;margin-bottom:6px;" onerror="this.style.display='none'"/><p style="font-size:11px;color:#666;">${CO.svcs.map((s,i)=>`${i>0?' 
  ...
  <div class="toolbar"><button class="btn-print" id="btnPrint">
  ...
  <div style="display:flex;justify-content:flex-end;margin-bottom:24px;"><div style="min-width:280px;font-size:13px;">
    ...
    ${quittung?`<div style="background:#dcfce7;border-radius:6px;padding:8px 12px;text-align:center;font-weight:700;color:#16a34a;font-size:13px;margin-top:8px;">
  </div></div>
...
function buildMonthlySummaryHtml({ label, total, offen, bezahlt, methods }) {
  ...
  <div class="bar"><button class="btn-print" id="btnPrint">
  <div class="sheet">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;border-bottom:3px solid #1a2744;padding-bottom:16px;margin-bottom:20px;">
      <div><img src="/logo_full.png" alt="Logo" style="height:55px;width:auto;margin-bottom:6px;" onerror="this.style.display='none'"/><p style="font-size:11px;color:#666;">${CO.svcs.join(' 
      ...
      <div style={{ display: 'grid', gap: 16 }}>
        ...
        <div className="card" style={{ borderRadius: 22 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div><label>Zahlungsstatus</label><select value={zahlstatus} onChange={e => setZahlstatus(e.target.value)} style={{ borderColor: zahlstatus === 'Bezahlt' ? '#16a34a' : '#d97706', color: zahlstatus === 'Bezahlt' ? '#16a34a' : '#d97706', fontWeight: 600 }}><option value="Offen">
          ...
          {zahlstatus === 'Bezahlt' && <div style={{ marginTop: 12, background: '#dcfce7', border: '1px solid #86efac', borderRadius: 12, padding: '10px 14px', fontSize: 13, color: '#16a34a', fontWeight: 700 }}>
        </div>
        ...
        <div className="card" style={{ borderRadius: 22, marginBottom: 14 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', gap: 16, flexWrap: 'wrap' }}>
            <div>
              ...
              <p style={{ margin: '6px 0 0', fontSize: 13, color: '#6b7280' }}>Total: {fmtCHF(selectedSummary.total)} 
            </div>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 14, flexWrap: 'wrap' }}>
            <button onClick={downloadAllPdf} style={{ background: '#1a2744', color: 'white', border: 'none', padding: '10px 14px', borderRadius: 12, fontWeight: 800 }}>
          </div>
        </div>
      )}
      {filtered.length === 0 ? (
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
            <div style={{ flex: 1, minWidth: 180 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                ...
                <span className={`badge badge-${inv.zahlstatus === 'Bezahlt' ? 'paid' : 'open'}`}>{inv.zahlstatus === 'Bezahlt' ? '
              </div>
              <p style={{ margin: 0, fontSize: 12, color: '#6b7280' }}>{inv.nr} · {fmtD(inv.datum)} 
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              ...
              <button className="btn-danger" onClick={() => onDelete(inv)} style={{ padding: '7px 12px', fontSize: 13 }}>
            </div>
          </div>
        </div>
      ))}
...
export default function App() {
  ...
  if (user === undefined) return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg,#1e2d4f,#2f4b88)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ color: 'white', textAlign: 'center' }}><div style={{ fontSize: 48, marginBottom: 16 }}>
    </div>
  )
    ...
    <div style={{ minHeight: '100vh', background: '#f4f6fb' }}>
      <header style={{ background: 'linear-gradient(135deg,#1e2d4f,#2f4b88)', color: 'white', padding: '0 20px', position: 'sticky', top: 0, zIndex: 100, boxShadow: '0 10px 30px rgba(30,45,79,.18)' }}>
        <div style={{ maxWidth: 1120, margin: '0 auto', height: 64, display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <button onClick={handleLogout} style={{ background: 'rgba(255,255,255,0.12)', color: 'white', border: '1px solid rgba(255,255,255,0.2)', padding: '8px 14px', fontSize: 13, borderRadius: 12 }}>
        </div>
      </header>
      <div style={{ background: 'white', borderBottom: '1px solid #e5e7eb', padding: '14px 20px' }}>
        <div style={{ maxWidth: 1120, margin: '0 auto' }}>
          <h1 style={{ margin: 0, fontSize: 18, fontWeight: 800, color: '#1a2744' }}>
            {screen === 'overview' && ' Übersicht'}{screen === 'new' && '+ Neue Rechnung'}{screen === 'edit' && '
          </h1>
        </div>
      </div>
      <main style={{ padding: '20px 16px', maxWidth: 1120, margin: '0 auto' }}>
        {loading ? (
          <div style={{ textAlign: 'center', padding: 70 }}><div style={{ fontSize: 40, marginBottom: 16 }}>
        ) : screen === 'overview' ? (
