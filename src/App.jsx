...
function buildPrintHtml(inv) {
  ...
  <table style="margin-bottom:12px;"><thead><tr><th style="width:40px;">Pos</th><th>Bezeichnung</th><th class="right" style="width:60px;">Std.</th><th class="right" style="width:90px;">CHF/h</th><th class="right" style="width:120px;">Positionspreis</th></tr></thead><tbody>
  ${inv.positionen.map((pos,idx)=>`<tr><td style="color:#888;">${idx+1}</td><td><strong>${pos.leistung}</strong>${pos.beschreibung?' – '+pos.beschreibung:''}</td><td class="right">${pos.stunden} h</td><td class="right">${Number(pos.ansatz).toFixed(2)}</td><td class="right" style="font-weight:700;">${fmtCHF(Number(pos.stunden)*Number(pos.ansatz))}</td></tr>`).join('')}
  </tbody></table>
  ...
  <div style="border-top:1px solid #ddd;padding-top:14px;font-size:11px;color:#666;line-height:1.8;">
    ...
    <div style="display:grid;grid-template-columns:160px 1fr;gap:2px 12px;">
      <span style="font-weight:600;">Zahlungskonditionen:</span><span>Barzahlung vor Ort nach Erledigung des Auftrags. Alle Preise zzgl. MwSt. 8.1%</span>
      <span style="font-weight:600;">Im Preis inbegriffen:</span><span>Betriebshaftpflichtversicherung (CHF 5'000'000.–), Transportversicherung (CHF 50'000.–), keine Benzin- oder Kilometerkosten, keine LSVA</span>
      <span style="font-weight:600;">Vertragsrücktritt:</span><span>30% ab 48h · 60% ab 14 Tage · 100% ab 7 Tage vor geplantem Einsatz</span>
    </div>
  </div>
  <div style="margin-top:28px;font-size:12px;color:#555;">
    ...
    <p style="margin:0;">${CO.title}</p><p style="margin:0;font-style:italic;">${CO.name}</p>
  </div>
            ...
            <div key={pos.id} style={{ background: '#f8fafc', borderRadius: 18, padding: 16, marginBottom: 12, border: '1px solid #e8edf3' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '170px 1fr', gap: 10, marginBottom: 10 }}>
                ...
                <div>
                  ...
                  <input placeholder={pos.leistung === 'Sonstiges' ? 'Was wurde gemacht?' : 'z.B. 2 Schränke, 3. OG ohne Lift…'} value={pos.beschreibung} onChange={e => ucp(pos.id, 'beschreibung', e.target.value)} style={{ borderColor: pos.leistung === 'Sonstiges' && !pos.beschreibung ? '#dc2626' : undefined }} />
                </div>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
                <div><label>{qtyLabel(pos)}</label><input type="number" min="0.5" step={qtyStep(pos)} value={pos.stunden} onChange={e => ucp(pos.id, 'stunden', e.target.value)} style={{ textAlign: 'right' }} /></div>
                ...
                <div><label>Positionspreis</label><div style={{ padding: '9px 12px', background: 'white', border: '1px solid #e2e5ec', borderRadius: 12, fontWeight: 800, textAlign: 'right', fontSize: 14, color: '#1a2744' }}>{fmtCHF(Number(pos.stunden) * Number(pos.ansatz))}</div></div>
              </div>
            </div>
          ))}
      ...
      {selectedSummary && (
        <div className="card" style={{ borderRadius: 22, marginBottom: 14 }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 14, flexWrap: 'wrap' }}>
            <button onClick={openPdf} style={{ background: '#1a2744', color: 'white', border: 'none', padding: '10px 14px', borderRadius: 12, fontWeight: 800 }}>🖨️ Monatsbilanz PDF</button>
            <button onClick={downloadAllPdf} style={{ background: '#0ea5e9', color: 'white', border: 'none', padding: '10px 14px', borderRadius: 12, fontWeight: 800 }}>⬇️ Alle als PDF</button>
          </div>
        </div>
      )}
